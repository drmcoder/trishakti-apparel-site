# Buyer outreach kit — paste-and-send (updated 2026-07-13)

Everything you need to reach overseas buyers directly, aligned to the LIVE site.
Keep pitches consistent with the site: same duty-free angle, same product story,
same ONE contact identity below.

## ✅ One identity — use everywhere (reconciled)
- **Email:** admin@trishaktiapparel.com.np  ← single primary export inbox (send + receive live)
- **WhatsApp:** +977 9863618347
- **Website:** https://trishaktiapparel.com.np
- **Headcount framing:** say **"a ~100-machine single-floor knit line, 80% women"** —
  do NOT quote a worker headcount until confirmed (old "~50 workers" line is retired to
  avoid an inconsistency). Machines + the 80% figure are both confirmed and stronger.

## Credibility links to drop into emails (all live now)
- Duty-free proof: https://trishaktiapparel.com.np/why-nepal/
- Honest compliance posture: https://trishaktiapparel.com.np/compliance/
- Ethics / 80% women: https://trishaktiapparel.com.np/sustainability/
- "Why trust a Nepali supplier": https://trishaktiapparel.com.np/blog/why-trust-a-nepali-garment-supplier/
- Products / line card: https://trishaktiapparel.com.np/products/

## Duty-free hook by market (lead with the right one)
- **EU:** EBA — duty-free, single transformation (Chinese fabric qualifies). LDC window to ~2029.
- **UK:** **DCTS** — duty-free incl. garments from imported fabric. ← strongest UK hook.
- **Canada:** LDCT — most fabric-friendly rule of all.
- **US:** no LDC duty-free — lead with quality / low MOQ / ethics / lowest-S.Asia tariff, not duty.

---

## READY EMAIL TEMPLATES (complete — personalise the [brackets], attach line-card + 3–5 photos)

### A. EU importer / brand
> **Subject:** Duty-free knitwear from Nepal — your designs, 0% EU duty
>
> Dear [Name],
>
> I'm Santosh Rijal, founder of Trishakti Apparel, a private-label knit garment factory in
> Gaindakot, Nepal. We make t-shirts, polos, and jersey basics to our customers' own designs
> and tech packs — a ~100-machine single-floor line, 80% women, with in-house CAD.
>
> The reason I'm writing to you specifically: as a Least Developed Country producer, our
> garments enter the EU at **0% import duty** under Everything But Arms — even with imported
> fabric, under single-transformation rules — versus the ~12% you'd pay from most origins.
> We supply the Certificate of Origin.
>
> We're built for programs like yours: a flexible MOQ (500–1,000 pcs/style), dedicated
> capacity, and direct communication. We're honest about where we stand on compliance —
> everything is on our site, nothing hidden.
>
> Could I send our line card and a few product samples? Happy to set up a call or a live
> video walkthrough of our floor.
>
> Warm regards,
> Santosh Rijal — Trishakti Apparel
> admin@trishaktiapparel.com.np · WhatsApp +977 9863618347 · trishaktiapparel.com.np
> Duty-free detail: trishaktiapparel.com.np/why-nepal/

### B. UK importer (DCTS angle)
> **Subject:** Nepal knitwear supplier — duty-free UK entry (DCTS), own designs
>
> Dear [Name],
>
> I'm Santosh Rijal, founder of Trishakti Apparel, a private-label knit factory in Nepal
> (t-shirts, polos, jersey basics to your tech packs). Under the UK's Developing Countries
> Trading Scheme, our Nepal-made garments enter the **UK duty-free** — including garments cut
> and sewn from imported fabric. That's a direct landed-cost advantage on every unit.
>
> We're a focused, quality-first floor (~100 machines, 80% women, in-house CAD) with a
> flexible MOQ of 500–1,000 pcs/style — well suited to growing and mid-size UK labels that want
> attention rather than a slot behind a mega-buyer.
>
> May I send our line card and free samples? A short call would be great too.
>
> Best,
> Santosh Rijal — Trishakti Apparel
> admin@trishaktiapparel.com.np · WhatsApp +977 9863618347 · trishaktiapparel.com.np

### C. US importer (quality + ethics + de-risk angle — NO duty-free claim)
> **Subject:** Ethical Nepal knit factory — low MOQ, China-plus-one option
>
> Dear [Name],
>
> I'm Santosh Rijal, founder of Trishakti Apparel, a private-label knit garment factory in
> Nepal. We make t-shirts, polos, and jersey basics to our customers' designs — a ~100-machine
> line that's 80% women, with fair wages and an open-door, audit-welcome policy.
>
> For US buyers diversifying away from concentrated sourcing, Nepal is a strong China-plus-one:
> among the lowest tariffs in South Asia, a flexible MOQ (500–1,000 pcs/style), and a
> partner who isn't overbooked. We're candid about compliance — what we hold and what's in
> progress is all on our site.
>
> Could I send our line card and samples, or set up a quick call?
>
> Best regards,
> Santosh Rijal — Trishakti Apparel
> admin@trishaktiapparel.com.np · WhatsApp +977 9863618347 · trishaktiapparel.com.np
> Ethics / 80% women: trishaktiapparel.com.np/sustainability/

### D. WhatsApp / short fast-fire (any market)
> Namaste 👋 I'm Santosh Rijal, founder of Trishakti Apparel — a private-label knit factory
> in Nepal (t-shirts, polos, sweatshirts & hoodies to your designs; MOQ 500–1,000 pcs; 80% women floor).
> We enter the EU/UK/Canada duty-free under Nepal's LDC status. Happy to send our line card,
> photos, or free samples — are you sourcing knitwear? trishaktiapparel.com.np

---

## FOLLOW-UP CADENCE (most replies come from touch 2–3, not touch 1)
- **Touch 1 — Day 0:** the email above + line card + 3–5 photos.
- **Touch 2 — Day 3–4:** short WhatsApp or reply-bump. "Following up — did the line card reach
  you? Happy to send free samples for [their product type]." Add ONE new proof link
  (e.g. /compliance/ or /why-nepal/).
- **Touch 3 — Day 8–10:** value-add, not a nag. Share the relevant blog post
  (duty-free guide, or "how to import from Nepal") + offer a live video floor tour.
- **Touch 4 — Day 20:** soft close. "I'll stop here so I'm not crowding your inbox — whenever
  you're reviewing knitwear sourcing, we'd love a look. Samples always free."
- Log every send/reply in a simple sheet (buyer, market, date, touch #, status).

## THE REAL BOTTLENECK (unchanged): assets convert, not more messages
Buyers convert on **photos + samples + a clean line card**, not extra emails.
- Line card one-pager: docs/line-card.html (print to PDF; attach to every email).
- Consider hosting a downloadable line-card PDF on the site (a "Download line card" button
  on /products/ doubles as a lead magnet). ← next conversion asset.

---

## VERIFIED WARM CONTACTS (from research 2026-07-11 — still valid)

### 1. Khangri Sourcing ⭐ warmest (Kathmandu office, local-to-local)
- Email: **info@khangrisourcing.com** — first email SENT ✅
- US line: **+1 425-251-0760** (not WhatsApp) · Kathmandu landline: **+977-1-4427191**
  (Radhe Marga, Dillibazar). Nepal ops via **Sherpa Adventure Gear Pvt Ltd**.
- Sherpa mobiles (likely WhatsApp): **+977 986-4111835**, **+977 982-3383447**
- **Next action:** CALL +977-1-4427191, reference the email, ask for WhatsApp, offer a visit.
  Local Nepali-to-Nepali call = fastest conversion.

### 2. Bradshaw Taylor (UK) — use template B (DCTS)
- Email: **info@bradshawtaylor.com** (+ dpo@bradshawtaylor.com); also the "Sales" contact form.
- Phone: **+44 1572 772436** · 16 Mill St, Oakham, Rutland, LE15 6EA, UK

### 3. Sudara (USA) — use template C; longer shot (mostly produces in India)
- Email: **Info@sudara.org** · Phone: **+1 949-403-0893** · Prefers **sudara.org/partner-with-us**

## DIRECTORIES / LISTINGS (discovery + backlinks — submit for SEO too)
- **TEPC** — info@tepc.gov.np · Na-Tole, Pulchowk, Lalitpur → REX number + directory listing.
- **GAN (Garment Association Nepal)** — gan@asso.wlink.com.np, gan@ntc.net.np · ganasso.org ·
  New Baneshwor · WhatsApp 9761801580 → membership + member listing (backlink).
- **Sagarmatha Export Import House** (Thamel) — Mr Shreedhar Regmi / Mr Sharad (buying house).
- Also worth listing on: Go4WorldBusiness, Fibre2Fashion marketplace, Kompass, Europages,
  TradeIndia — each is a discovery channel AND a backlink to trishaktiapparel.com.np.
